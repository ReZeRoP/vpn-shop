...
export default function SettingsForm({ initial }: { initial: Record<string, string> }) {
  ...
  async function testPanel() {
    setTestResult("در حال آزمایش...");
    const res = await fetch("/api/admin/settings/test-panel", { method: "POST" });
    const data = await res.json().catch(() => ({}));
    setTestResult(res.ok ? `✓ اتصال موفق — ${data.inbounds} اینباند یافت شد: ${data.list}` : `✗ ${data.error}`);
  }
  ...
  return (
    <div className="max-w-xl space-y-4">
      ...
      <div className="flex items-center gap-3 pt-2">
        ...
        <button
          onClick={testPanel}
          className="rounded-xl border border-line px-6 py-2.5 text-sm text-muted hover:text-fg"
        >
          آزمایش اتصال پنل
        </button>
      ...
      {testResult && (
        <p className={`text-sm ${testResult.startsWith("✓") ? "text-ok" : "text-danger"}`}>{testResult}</p>
      )}
    </div>
  );
}
